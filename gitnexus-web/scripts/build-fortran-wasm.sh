...
# Requires: git, node, tree-sitter CLI (npm install -g tree-sitter)
set -e
...
echo "Building WASM (requires tree-sitter CLI: npm install -g tree-sitter)..."
tree-sitter build --wasm -o tree-sitter-fortran.wasm 2>/dev/null || npx tree-sitter build --wasm -o tree-sitter-fortran.wasm
